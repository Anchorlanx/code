...
local function getCorners(guiObject0)
	...
	local atan = math.atan2(size.Y, size.X)
	local cos = math.cos
	local sin = math.sin
	local rad = math.rad
	local sqrt = math.sqrt
	local halfDiag = sqrt((size.X/2)^2 + (size.Y/2)^2)

	local a = pos + size/2 - halfDiag * Vector2.new(cos(rad(rotation) + atan), sin(rad(rotation) + atan))
	local b = pos + size/2 - halfDiag * Vector2.new(cos(rad(rotation) - atan), sin(rad(rotation) - atan))
	local c = pos + size/2 + halfDiag * Vector2.new(cos(rad(rotation) + atan), sin(rad(rotation) + atan))
	local d = pos + size/2 + halfDiag * Vector2.new(cos(rad(rotation) - atan), sin(rad(rotation) - atan))
...
local function draggable(obj, extern, parented)
	...
	obj.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 and souid == false and obj.ZIndex >= index then
			souid = true
			index = obj.ZIndex + 1  -- Bring to front
			obj.ZIndex = index
			obj.Parent = PCR_1
		...
		elseif input.UserInputType == Enum.UserInputType.MouseButton1 and souid == false and obj.ZIndex < index then
			souid = true
			index = obj.ZIndex + 1  -- Bring to front
			obj.ZIndex = index
			obj.Parent = PCR_1
			...
			connection = input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					...
					souid = false
					connection:Disconnect()
					-- Removed buggy line: if isColliding(obj, obj.Section) then obj = obj.Section end
					-- Assuming it was a typo; if needed, implement proper snapping
				end
			end)
		end
	end)
...
function OpenedColor(text, ColourDisplay, Action, def)
	...
	local uis = game:GetService("UserInputService")
	colorPickerSingleton.ANIMATEFRAME01.Visible = true
	colorPickerSingleton.Visible = true
	...
	colorPickerSingleton.ANIMATEFRAME01:TweenPosition(UDim2.new(-0.026,0,-0.007,0),'Out','Quint',0.2,true)
	task.wait(0.2)
	colorPickerSingleton.BackgroundTransparency = 0
	colorPickerSingleton.Holder.Visible = true
	colorPickerSingleton.ANIMATEFRAME01:TweenPosition(UDim2.new(1.077,0,-0.007,0),'Out','Quint',0.2,true)
	task.wait(0.2)
	colorPickerSingleton.ANIMATEFRAME01.Position = UDim2.new(-1.085,0,-0.013,0)
...
function AddRipple(button, ael, ayo)
	ayo = ayo or Color3.fromRGB(197, 197, 197)
	...
	local background = button:WaitForChild("Background", 1) or Background  -- Fallback
...
function library:AddWindow(text)
	...
	local function UpdateMainSize(f, anim)
		...
		local size = larger == 'l' and getlarger(2).l or getlarger(2).r
		if anim then
			TweenService:Create(HOLDER, TweenInfo.new(0.3, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {CanvasSize = UDim2.fromOffset(0, size + 15)}):Play()
		else
			HOLDER.CanvasSize = UDim2.fromOffset(0, size + 15)
		end
	end
	...
	local function GetSide(typ, input)   
		...
		elseif typ == 2 and input then
			if typeof(input) == "string" then   
				if input:lower() == 'right' or input:lower() == 'r' then
					return _RIGHT, RUIL
				elseif input:lower() == 'left' or input:lower() == 'l' then
					return _LEFT, LUIL
				end
	...
	function sec:AddSection(Texto, side)
		...
		if library.sections[SECTIONHOLDER.Name] then
			warn('WARNING: Sections must have unique names!')
			return 
		end
		...
		function inside:AddTextBox(Text, placeholder, CTOF, Type, Action)
			...
			local filter2 = '%p+'
			local onlunum = '%D+'   
			local onlychars = '[^a-zA-Z]+'  -- Fixed to remove non-letters for type 4

			local function colador(str, type)
				local str = str
				if type == 1 then
					str = str:gsub(onlunum, '')  -- Keep digits only
				elseif type == 2 then
					str = str:gsub(filter2, '')  -- Remove punctuation
				elseif type == 3 then
					str = str:gsub(filter, '')  -- Remove non-alphanum
				elseif type == 4 then
					str = str:gsub(onlychars, '')  -- Keep letters only
				elseif type == 5 then
					-- No filter
				end
				return str
			...
			local AC = function(PassBox)
				PassBox.Text = colador(PassBox.Text, Type)
				...
				if #PassBox.Text >= 21 then
					PassBox.Text = string.sub(PassBox.Text, 1, 21)
				end
				task.spawn(function()
					pcall(Action, PassBox.Text)
				end)
			end

			local text = ''
			obj5.Changed:Connect(function()
				if obj5.Text == '' or obj5.Text == ' ' then
		...
		function inside:AddSlider(Text, Max, Min, def, Action)
			...
			local obj4 = Instance.new("TextBox", obj1)
			obj4.Name = "pcntage"
			...
			obj4.FocusLost:Connect(function(n)
				if n then
					if obj4.Text == '' or obj4.Text:find(' ') then
						Value = tonumber(obj4.Text) or st
						obj4.Text = tostring(st)
					else
						Value = math.clamp(tonumber(obj4.Text), Min, Max)
		...
		local function getsize(str)
			local r = #str
			if r <= 5 then
				return r * (50 / r)  -- Adjusted for better sizing
			end
			return r * 7.5
		end
		...
		function inside:AddToggle(Text, Enabled, keybind, Callback)
			...
			KeyButton.Text = keybind and keybind.Name or '. . .'
			...
			if keybind then
				...
				local KeyCode = keybind.Name
				uis.InputBegan:Connect(function(a, gp)
					if not gp and not ischanging and a.KeyCode.Name == KeyCode then
						Update()
					end
				end)

				KeyButton.MouseButton1Click:Connect(function()
					TweenService:Create(KeyButton, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {BackgroundColor3 = Color3.fromRGB(34, 34, 34)}):Play()
					KeyButton.Text = ". . ."
					KeyButton:TweenSize(UDim2.new(0, getsize(KeyButton.Text), 0, 15), "InOut", "Quint", 0.2, true)

					local v1 = uis.InputBegan:Wait()
					if v1.KeyCode.Name ~= "Unknown" then
						ischanging = true
						TweenService:Create(KeyButton, TweenInfo.new(0.1, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {BackgroundColor3 = Color3.fromRGB(25, 25, 25)}):Play()
						KeyButton:TweenSize(UDim2.new(0, getsize(v1.KeyCode.Name), 0, 15), "Out", "Quint", 0.3, true)
			...
			AddRipple(Interactive, TextLabel, Color3.fromRGB(197, 197, 197))  -- Added ripple for consistency
		...
		function inside:AddColorPallete(Text, Color, Action)
			...
			AddRipple(OPENCLOSE, OPENCLOSE, Color3.fromRGB(197, 197, 197))  -- Added ripple
		...
		function inside:AddKeyBind(Text, KeyCode, Action)
			...
			local ischanging = false
			uis.InputBegan:Connect(function(a, gp)
				if not gp and not ischanging and a.KeyCode == KeyCode then
					pcall(Action, a.KeyCode)
				end
			end)
			...
			AddRipple(Interactive, TextLabel, Color3.fromRGB(197, 197, 197))  -- Added ripple
		...
		function inside:AddDropdown(Text, tbl, sel, Action)
			...
			for i, v in pairs(tbl) do
				...
				OPTION.MouseButton1Click:Connect(function()
					for _, opt in pairs(DRPDOWN:GetChildren()) do
						if opt:IsA("TextButton") and opt ~= Toggle then
							TweenService:Create(opt, TweenInfo.new(0.26, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundColor3 = Color3.fromRGB(37, 37, 37)}):Play()	
						end	
					end
					s = TextLabel_3.Text
					TweenService:Create(OPTION, TweenInfo.new(0.26, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {BackgroundColor3 = Color3.fromRGB(28, 28, 28)}):Play()	
				...
				AddRipple(OPTION, TextLabel_3, Color3.fromRGB(197, 197, 197))
			...
			AddRipple(Toggle, TextLabel_2, Color3.fromRGB(197, 197, 197))
		...
		function inside:AddButton(Text, Callback)
			...
			AddRipple(Interactive, TextLabel, Color3.fromRGB(197, 197, 197))  -- Ensure button has ripple effect
